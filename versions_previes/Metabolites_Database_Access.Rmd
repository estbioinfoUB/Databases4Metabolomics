---
title: "Metabolites Database Access"
author: "Alex Sanchez"
date: "2025-10-23"
...
This has been processed elsewhere, using an ad-hoc function (`build_SMPDB.R`) that generates a list with an item per pathway, each of wich  which contains a (nested) list with all the associated metabolite identifiers.
...
# 10) Guardar objetos .rda para uso con enrichmet()
# 10) Construir EnrichmentSet per a KEGG (global i study)

# Convertir la taula de sets a un format list-style com els de SMPDB
KEGG_list_global <- list(
  name = "KEGG_pathway_hsa_global",
  description = "Human KEGG pathways mapped to HMDB via union(study, metaboliteIDmapping)",
  version = as.character(Sys.Date()),
  sets = setNames(
    strsplit(KEGG_list_global$sets$Metabolites, ",", fixed = TRUE),
    KEGG_list_global$sets$PathwayID
  )
)

KEGG_list_study <- list(
  name = "KEGG_pathway_hsa_study",
  description = "Human KEGG pathways mapped to HMDB, restricted to study universe",
  version = as.character(Sys.Date()),
  sets = setNames(
    strsplit(KEGG_list_study$sets$Metabolites, ",", fixed = TRUE),
    KEGG_list_study$sets$PathwayID
  )
)

# Crear EnrichmentSet amb la funció del paquet localEnrichment
library(localEnrichment)

KEGGset_global <- as.EnrichmentSet.list(KEGG_list_global)
KEGGset_study  <- as.EnrichmentSet.list(KEGG_list_study)

# Guardar
save(KEGGset_global, file = "results/KEGG_EnrichmentSet_global.rda")
save(KEGGset_study,  file = "results/KEGG_EnrichmentSet_study.rda")

message("KEGG EnrichmentSets guardats:")
message(" - results/KEGG_EnrichmentSet_global.rda (", nrow(KEGGset_global@data), " pathways)")
message(" - results/KEGG_EnrichmentSet_study.rda (",  nrow(KEGGset_study@data),  " pathways)")

# 11) Smmary message
cat("\nTop pathways according to study coverage:\n")
print(head(sets_diag, 10))
...
```{r}
KEGGset_study_df <- KEGGset_study$sets
KEGGset_global_df <- KEGGset_global$sets
```
...
Although chemical classess is a less rich source of sets a Metabolite Set based on them can be built in a similar way as was done with KEGG.
...
```{r}
ChemicalClassSet_df <- as.MetaboliteSetDataFrame (ChemicalClassSet)
...
The list can be easily converted into an `EnrichmentSet` object:
...
The `EnrichmentSet` objects can now converted into a `data.fram`, which is the expected structure for enrichment analysis

```{r}
SMPDBset_df <- as.MetaboliteSetDataFrame(SMPDBset)
dim(SMPDBset_df)
head(SMPDBset_df)
```
...
```{r}
SMPDBset_study_df <- as.MetaboliteSetDataFrame(SMPDBset_study)
...
As a summary of all this exploration, we have generated several collections of `MetaboliteSets` as both data.frames and EnrichmentSets that can be saved for later use in Enrichment Analysis:

- `KEGGset_global_df` and `KEGGset_global`
- `ChemicalClassSet_df` and `ChemicalClassSet`
- `SMPDBset_df` and `SMPDBset`
...
```{r}
save(KEGGset_global_df, KEGGset_global, 
     ChemicalClassSet_df,  ChemicalClassSet, 
     SMPDBset_df, SMPDBset,
     file="results/MetaboliteSets_Collection.Rda")
```


Thes metabolites sets can be manipulated, e.g. converted into/from lists or filtered based on a list of ids with the functions available in package `localEnrichment`
